...
app_id = "yourAPPid"
app_secret = "yourappscret" # DO NOT SHARE WITH ANYONE!
...
def frame(data):
    ...
    print l
    df = pd.DataFrame(
        columns=['id', 'link', 'typ', 'message', 'created_time', 'shares', 'comments', 'likes', 'love', 'haha', 'wow',
                 'sad', 'angry'])
    ...
    while (p >= 0):
        ...
        x = len(url) - 1
        url = url[
              0:x] + "&fields=id,link,type,message,created_time,shares,comments.limit(0).summary(total_count),reactions.type(LIKE).limit(0).summary(total_count).as(reactions_like),reactions.type(LOVE).limit(0).summary(total_count).as(reactions_love),reactions.type(WOW).limit(0).summary(total_count).as(reactions_wow),reactions.type(HAHA).limit(0).summary(total_count).as(reactions_haha),reactions.type(SAD).limit(0).summary(total_count).as(reactions_sad),reactions.type(ANGRY).limit(0).summary(total_count).as(reactions_angry)&access_token=1774340462813774|806f62247d55390ef50528a17c5fb514"
        print url
        next_page = True
        datalist = []
        data = json.loads(GetInitialPageData(url))
        ...
        for posts in data:
            ...
            try:
                datalist.append(data[posts]["message"])
            ...
            try:
                datalist.append(data[posts]["reactions_love"]["summary"]["total_count"])
            except:
                datalist.append(0)

            try:
                datalist.append(data[posts]["reactions_haha"]["summary"]["total_count"])
            except:
                datalist.append(0)

            try:
                datalist.append(data[posts]["reactions_wow"]["summary"]["total_count"])
            except:
                datalist.append(0)

            try:
                datalist.append(data[posts]["reactions_sad"]["summary"]["total_count"])
            except:
                datalist.append(0)

            try:
                datalist.append(data[posts]["reactions_angry"]["summary"]["total_count"])
            except:
                datalist.append(0)

            s = pd.Series(datalist,
                          index=['id', 'link', 'typ', 'message', 'created_time', 'shares', 'comments', 'likes', 'love',
                                 'haha', 'wow', 'sad', 'angry'])
...
def process():
    ...
    s = "CREATE TABLE "+tablename+"(id char(255),link TEXT,typ CHAR(255),message TEXT,created_time INT,shares INT ,comments INT ,likes INT,love INT ,haha INT ,wow INT ,sad INT ,angry INT )"
    ...
    s = "CREATE TABLE "+table2+"(id char(255),link TEXT,typ CHAR(255),message TEXT,created_time INT,shares INT ,comments INT ,likes INT,love INT ,haha INT ,wow INT ,sad INT ,angry INT )"
